...
- ~~`allowedModelIds`/`allowedThinkingLevels` が Swift ソースにハードコードされているため、真に新規のモデルIDへの切替は依然としてアプリのソース変更・再ビルドを要する（ADR-011 のシナリオそのものを完全には解決しない）~~ → **[ADR-014](./ADR-014-vertex-ai-model-denylist.md) で解消**（modelId は完全一致allowlistからProhibited denylistへ改訂、真に新規のモデルIDもアプリ再ビルドなしで運用可能に）
