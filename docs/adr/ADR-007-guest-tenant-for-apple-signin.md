...
| 2 | Apple Refresh Token の revoke 未実装。Guideline 5.1.1(v) は revoke を要求するが、auth code は 5分TTLで再認証導線の追加工数が大きい。過去6回の審査は 2.1(a) リジェクトのみで 5.1.1(v) 違反は未経験のため Build 33 はスコープ外 | Issue化予定 |
