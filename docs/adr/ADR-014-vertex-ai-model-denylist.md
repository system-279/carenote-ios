...
1. **denylistが拒否するのは以下の2種類のみ**:
   - (a) 無印「Gemini 3 Flash」ベースモデル: `gemini-3-flash` 完全一致、または `gemini-3-flash-` で始まるID。マイナーバージョン付き（`gemini-3.5-flash`・`gemini-3.6-flash`・`gemini-3.1-flash-lite` 等）は対象外——ADR-011 が既に「`gemini-3.5-flash` は Prohibited の対象外」と判断した解釈をそのまま踏襲する
   - (b) preview/experimental系: ハイフン区切りの構成要素（`split(separator: "-")`）に `preview` / `exp` / `experimental` のいずれかを含むもの
2. **判定はハイフン区切りの完全な構成要素一致とし、部分文字列一致は行わない**。`contains("exp")` のような単純な部分一致だと、将来 `exp` を含む無関係な語（例: `expansion` 等）が偶発的に混入した場合に誤検知しうるため、`split(separator: "-")` した各要素との完全一致で判定する
...
4. **出荷時デフォルトは `gemini-3.5-flash` を維持、`gemini-3.6-flash` へは切り替えない**（ただし根拠は当初の記述から訂正、下記参照）。`gemini-3.5-flash` は CareNote が実際に使う Vertex AI `generateContent` API（`aiplatform.googleapis.com`）で asia-northeast1 の on-demand 呼び出しを実測済み（2026-07-20、HTTP 200 + `trafficType: "ON_DEMAND"`、実プロジェクトのGCP環境から直接呼び出して確認）。`gemini-3.6-flash` は同APIでの asia-northeast1 データレジデンシー対応を **本ADR作成時点では確認できていない**（下記「訂正」参照）。介護記録という個人情報を扱うCareNoteでは、未確認のままDRZ非対応の可能性があるモデルへ性能・コスト優位だけで切り替えることはできないため、確定情報が得られるまで `gemini-3.5-flash` を維持する。この判断は自由入力式のメリットをまさに示す例でもある: `gemini-3.6-flash` の日本DRZ対応が確認・追加されれば、運営者がFirestore設定を変更するだけで審査なしに切替可能になる

   > **訂正（本ADR初版からの修正）**: 初版では `gemini-3.6-flash` の日本DRZ非対応を「`docs.cloud.google.com/gemini/enterprise/docs/locations` で確認済み」としていたが、このページは実際には **「Gemini Enterprise Standard/Plus Editions」という別製品**（CareNoteが使うVertex AI `generateContent` とは異なる製品ライン）のデータレジデンシーページであり、製品スコープの確認を怠っていた（AI駆動開発ハーネスの一般ルール `rules/tech-selection.md` §1.1「一次ソースの製品スコープ確認」に事例として追記済み）。正しい対象は `docs.cloud.google.com/vertex-ai/generative-ai/docs/learn/data-residency` だが、この訂正作業中にJSレンダリングでの表組み抽出不能・dev環境での実疎通テストも認証の reauth 要求（非対話実行不可）によりブロックされ、**`gemini-3.6-flash` の asia-northeast1 DRZ対応は本ADR時点で「未確認」のまま**。結論（`gemini-3.5-flash` 維持）自体は変わらない（未確認のモデルを個人情報プロジェクトのデフォルトに採用しないのは妥当な判断のため）が、根拠は「非対応と確認済み」ではなく「確認できておらず、確認できるまでは変更しない」に訂正する。次回、対話ログイン後の実疎通テストか、正しい製品ページの確認ができ次第、再評価する
...
- modelIdのフォーマットチェック（英数字・ドット・ハイフンのみ、空文字禁止）は行うが、実在性・リージョン対応・データレジデンシー適合性の検証は運営者の責任（`scripts/set-vertex-ai-config.sh`・GitHub Actions workflow のいずれも無検証で書き込む）

## 影響
- `CareNote/Models/VertexAIConfig.swift`: `allowedModelIds`（完全一致Set）を`isModelAllowed(_:)`（denylist判定関数）に置換。`allowedThinkingLevels`・`default`は変更なし
- `.github/workflows/firestore-op.yml`: `model_id` 入力を `type: choice`（`gemini-3.5-flash`固定）から `type: string`（自由入力）に変更。Validate inputsのbash再検証を同じdenylistロジックに置換（フォーマットチェック含む）。`thinking_level`は変更なし
- `CareNoteTests/VertexAIConfigServiceTests.swift`: denylist境界テストを追加（将来モデル名の採用・無印gemini-3-flash拒否・preview拒否・exp拒否）
