...
4. **出荷時デフォルトは `gemini-3.5-flash` を維持、`gemini-3.6-flash` へは切り替えない**: `gemini-3.6-flash` はGA済みだが、Google公式ロケーションページ（`docs.cloud.google.com/gemini/enterprise/docs/locations`）で **`global` リージョン限定、日本を含むデータレジデンシー(DRZ)対応の記載なし** と確認した（一次ソース直接確認、独立した別セッションの調査結果とも一致）。対して `gemini-3.5-flash` は同ページで日本(JP)を含むDRZ対応が明記されている。介護記録という個人情報を扱うCareNoteでは、性能・コストの優位だけでDRZ非対応モデルへ切り替えることはできない。この判断は自由入力式のメリットをまさに示す例でもある: `gemini-3.6-flash` の日本DRZ対応が将来追加されれば、運営者がFirestore設定を変更するだけで審査なしに切替可能になる
