...
- **文字起こし**: Vertex AI Gemini 3.5 Flash (`gemini-3.5-flash`, asia-northeast1, `thinkingLevel: minimal`)（[ADR-011](docs/adr/ADR-011-gemini-3-5-flash-migration.md)）
  - モデル名・thinkingLevel は `platformConfig/vertexAi`（Firestore、テナント非依存、admin SDK 経由のみ書込）から解決する（[ADR-012](docs/adr/ADR-012-vertex-ai-config-firestore.md)）。iOS 側 `VertexAIConfigService` が検証を通した値のみ採用し、未シード・不正値・fetch失敗時はアプリ同梱のハードコードデフォルトへソフトフェイルする
  - `CareNote/Models/VertexAIConfig.swift` の検証は modelId/thinkingLevel で方式が異なる（[ADR-014](docs/adr/ADR-014-vertex-ai-model-denylist.md)）: **modelId** は本ファイル Prohibited のモデル制約のみを拒否する denylist（正しいモデル名は運営者が自由入力すればアプリ更新なしで反映される）、**thinkingLevel** は `minimal` のみを許可する完全一致 allowlist（据置）。Prohibited を変更する場合はこの検証ロジックも同時に更新する
